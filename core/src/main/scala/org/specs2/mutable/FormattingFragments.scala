...
import org.specs2.specification.{StandardFragments, FragmentsFragment, Fragments, Fragment}
...
trait FormattingFragments extends specification.FormattingFragments { outer: FragmentsBuilder =>
  ...
  override def t(n: Int)  = { val f = StandardFragments.Tab(n) ; addFragments(f); f }
  override def bt         = { val f = super.bt   ; addFragments(f); f }
  override def bt(n: Int) = { val f = StandardFragments.Backtab(n); addFragments(f); f }
